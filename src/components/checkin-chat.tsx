    ...
    <div className="flex flex-col h-full max-h-[70vh]">
          ...
          <div key={i}>
            <div className={cn(
              ...
              <div className={cn(
                ...
                msg.role === "user" ? "bg-accent-100" : "bg-blue-100"
              )}>
                {msg.role === "user" ? <User className="h-4 w-4 text-accent-700" /> : <Bot className="h-4 w-4 text-blue-700" />}
              </div>
              <div className={cn(
                "rounded-xl px-3 py-2 text-sm max-w-[80%]",
                msg.role === "user" ? "bg-accent-600 text-white" : "bg-gray-100 text-gray-800"
              )}>
                {msg.content}
              </div>
            </div>
            ...
            {msg.progressSuggestions && msg.progressSuggestions.length > 0 && (
              <div className="ml-9 mt-2 space-y-1">
                {msg.progressSuggestions.map(s => (
                  <div key={s.topicId} className="flex items-center gap-2 bg-accent-50 rounded-lg px-3 py-1.5">
      ...
      <div className="border-t p-3 flex gap-2">
        <Input
          ...
          autoFocus
        />
        <Button onClick={sendMessage} disabled={loading || !input.trim()} size="icon" className="bg-accent-600 hover:bg-accent-700">
